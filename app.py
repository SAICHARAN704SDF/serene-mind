...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'sqlite:///serenemind.db')
...
def add_entry():
    ...
    import uuid
    entry = JournalEntry(id=str(uuid.uuid4()), text=text)
...
def add_mood():
    ...
    import uuid
    log = MoodLog(id=str(uuid.uuid4()), mood=mood)
    db.session.add(log)
    db.session.commit()
    return redirect(url_for('journal'))
